...
To address this, I really like the `viridis` package, which has very nicely curated palettes that are all colorblind-friendly. They also read nicely in grayscale and B&W.
...
▶️ A plot is only as good as its labels -- let's make ours clearer and more polished by setting them with `labs()`
```{r}
ggplot(
  small_star_wars,
  aes(
