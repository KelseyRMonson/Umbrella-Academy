...
![Click the arrow next to your Tibble's name to expand the details for a quick peek. <br>Or, click the little table icon on the right to open it in its own tab.](assets/Environment.png)
...
Try just calling the `ggplot()` function on its own:
```{r}
ggplot()
```
