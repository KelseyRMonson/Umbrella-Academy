...
To specify this, use the `aes()` argument to map the *aesthetics* -- like variables applied to axes or colors.
...
To address this, I really like the `viridis` package, which has nicely curated palettes that are colorblind-friendly. They also read well in grayscale and B&W.
...
▶️ Let's simplify the plot by removing `ggplot`'s default gray background. We can do this using a `theme()`:
...
ggplot(
  ...
  aes(                        # Define your aesthetics
    ...
    fill = species                # You can include lots of other things in aes()
  )                           
) +
  geom_col() +                       # Layers can be things like geom_col, geom_point(), geom_line(), geom_boxplot(), etc
  geom_point(                         # You can add multiple different types of geoms, or even multiple of the same type
    aes(color = homeworld, size = homeworld) # Map both color and size to the same variable (homeworld)
  ) +  
  scale_fill_viridis_d(option = "D", alpha = 0.8, begin = 0.2, end = 0.5) +
  scale_color_viridis_d(option = "C", alpha = 0.9, begin = 0.1, end = 0.9) +
  guides(
  ...
  labs(
    ...
    y = "Height (cm)",
    fill = "Species",
    title = "Star Wars Character Heights with Dynamic Scales",
