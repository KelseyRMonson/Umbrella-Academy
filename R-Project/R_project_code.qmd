...
R isn't reading and evaluating our character variables to assume what they might mean, so it isn't going to automatically convert `one` into `1`.
...
-   Double question mark: `??`
    ...
    -   This type search of works better if you use more specific keywords, since many functions share common words (in fact, you can try out `??mean()` and see how many results you get -- it might take a while!)
...
This is how people can use R to calculate differential gene expression from RNA-seq data, plot complex microbiome phylogenetic trees, use machine learning tools to make sophisticated predictive statistical models, and more. These tasks are all very different and require broad expertise to program and execute. But with the broad and brilliant user base of R, there are people who make packages to do each of these things (and so much more!).
