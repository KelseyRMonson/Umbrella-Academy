...
Since we already have a `none` category, and if we want to ensure our friend R2 is included in our hair color table, we could recode it so that an `NA` entry for hair becomes `none`:
...
plot(starwars_noHutt$height, starwars_noHutt$mass)
...
# I'm creating a scatter plot with mass and height on the x- and y-axes, colored by species, and with point size representing birth year:
p <- ggplot(starwars_noHutt, aes(x = mass, y = height, color = species, size = birth_year)) +
  geom_point() +
  ylim(NA, 230) +
  xlim(NA, 140) +
  scale_color_viridis(alpha = 0.7, begin = 0, end = 1, option = "D", aesthetics = "color", discrete = TRUE) +
  theme_light() +
      labs(
        x = "Weight",
        y = "Height"
