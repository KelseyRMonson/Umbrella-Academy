...
```{r fig.cap= "`birth year` is missing for several entries, so fewer points are plotted. <br><br>*That's a downside to plotting additional variables when there is missing data (which is most datasets!).*"}
#| code-fold: true
#| code-summary: "Show the code"
...
Because the mean of height is greater than the mean of weight (because they are on different scales), all the heights are greater than all the weights.
...
▶️ Pick your favorite plot from any point during the exercise. Copy the code, but now create a variable by creating a name and using the assignment operator `<-` before the `ggplot()` call:
