...
```{r fig.cap= "Note that `birth year` is missing for several entries, so this plot looks slightly different than the previous one, with fewer points plotted. <br><br>*That's a downside to layering on additional variables in an imperfect dataset with missing data (which most are!).*"}
#| code-fold: true
#| code-summary: "Show the code"
