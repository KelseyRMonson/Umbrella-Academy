...
Histograms are like bar plots, but for continuous variables instead of categorical ones.

Here, we're plotting `height` on the x-axis now, and counting the observations at each height grouping on the y-axis.
...
Those groupings are called "bins."
...
You can plot densities for several groups at once (you can do this for histograms too, but it's easier to visualize with density maps)
...
Scatter plots effectively visualize relationships between two continuous variables.

Let's recreate a simple of the one I showed you at the beginning of the lecture.

▶️ First plot height vs weight, mapping color onto weight
...
Like I did before, you can add additional information.
...
While you *can* use it to make heatmaps, the appropriately named `pheatmap` (for "Pretty Heatmap") is best.

Here's a simple heatmap comparing `height` and `weight` in our dataset:
...
First, `height` (cm) and `weight` (kg) are in different units, but the color scale is treating them the same.
...
By looking at the dendogram on the left, we can get a greater appreciation for which individuals it is grouping together.
...
Well, we can also see that Darth Vader and IG-88 are grouped together. While Darth Vader is "more machine than man" according to Obi-Wan, he's still a human, but is being grouped together with an assassin droid.
