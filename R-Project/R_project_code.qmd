...
```{r fig.cap="This is a simple scatter plot showing Star Wars charater Height vs. Weight using the Base-R `plot()` function. <br><br>*Note: I dropped Jabba the Hutt from this analysis since he's such a big boi.*"}
#| echo: false
...
```{r fig.cap="Note that `birth year` is missing for several entries, so this plot looks slightly different than the previous one, with fewer points plotted. <br><br>*That's a downside to layering on additional datapoints in an imperfect dataset with missing data (which most are!).*"}
#| code-fold: true
#| code-summary: "Show the code"
