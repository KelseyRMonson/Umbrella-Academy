...
The bin width corresponds to the size of the grouping on the x-axis. For `binwidth=100`, for example, each bin spans 100 cm.
...
Let's recreate a version of the one I showed you at the beginning of the lecture.

▶️ Starting with a simple one: height vs weight, mapping color onto weight
...
▶️ Let's map `species` to color and `birth year` to size:
```{r fig.cap= "Note that `birth year` is missing for several entries, so this plot looks different, with fewer points plotted. <br><br>*That's a downside to plotting additional variables when there is missing data (which is most datasets!).*"}
#| code-fold: true
#| code-summary: "Show the code"
...
### Heatmaps
We'll close by making a heatmap. This is one of the few instances where a plot from Base-R outshines one from `ggplot2`.
...
pheatmap(
  ...
  color = colorRampPalette(c("blue", "red"))(50) # Color gradient
)
