...
Now that we have the basic structure, let's see how to make several other common plots.

### Discrete Variables
Let's continue on with plotting discrete/categorical values.

#### More bar plots
We used `geom_col()` for our bar plot, but we could also use `geom_bar()`. 

If we use `stat="identity"` with `geom_bar()`, we can generate the same plot
...
▶️ Let's use `geom_bar()` to plot height by species, instead of name.
```{r}
#| code-fold: true
#| code-summary: "Show the code"
ggplot(
...
```
This sums the heights of all the humans in the dataset, which is not especially informative. 

So what's a better way to plot this?
...
Maybe you're interested in the species and sex distribution of height for our characters -- but now we're running into quite a few variables. That's where `facet_wrap()` comes in. It allows you to split out graphs with your variable of interest, to better visualize multi*faceted* data (get it?).
