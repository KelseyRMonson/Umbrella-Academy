...
You'll most often use **Data Frames** or **Tibbles** to create figures, but will use all the variable types at some point.
...
R isn't reading or understanding the words in our **character vector**, so it isn't going to automatically convert `one` into `1`.
:::
...
::: callout-caution
**R treats text variables alphabetically by default**.
...
We can set the levels to be whatever we want.

```{r}
# For example, we could reverse them:
...
R Packages let you do things like:

-   Calculate RNA-seq gene expression
-   Plot microbiome phylogenetic trees
-   Use machine learning to create statistical models
-   ...and more!
...
It's a good idea to load all of your packages at the beginning of your script. 
...
#### Vignettes
You can learn a lot about how to use a package using R's help system, but many bioinformatics packages also come with **vignettes**.

**Vignettes** are long-form, interactive guides to R packages. These give you a more holistic view of the workflows that you can complete with a package, instead of trying to parse each function individually.

As an example, here's the link to the [DESeq Vignette](https://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html) for RNA-seq differential expression analysis. 

To find them, I usually just Google "`<package name>` vignette" but there is also a `browseVignettes` function in R. Not *every* package will have one -- they are usually only created for robust, complicated packages. 
...
![Create a new project by going to File -\> New Project. Then create a New Directory, and finally a New Project.](assets/New_Proj.png)
...
I've created an [R Script](R_code/Getting_Started_with_R_script.R) with the code needed for this exercise.

::: callout-note
-   You can use this script as a template as you write your own script, but try typing out the commands to build muscle memory 💪
...
Go ahead and open that script now, and save it into the R Project directory you just made.

::: callout-tip
Headings make it much easier to navigate around long scrips in RStudio.

Open the `Document Outline` in this script (top right corner) to view and jump to specific sections. I almost always have this open when I'm working.

You can also jump to specific sections using the `#️⃣ Code Chunk Navigator` at the bottom of the script editor.

![](assets/Script_Navigation.png)
:::
...
▶️ You can also use the `gtExtras` package to create a polished summary table with very simple code:
...
```{r fig.cap="This is a simple scatter plot showing Star Wars character Height vs. Weight using the Base-R `plot()` function. <br><br>*Note: I dropped Jabba the Hutt from this analysis since he's such a big boi.*"}
#| echo: false
...
```{r fig.cap="This is the same Height vs. Weight scatter plot made with `ggplot2`. <br><br>I've also added boxplots showing the marginal distribution of each variable, and point size and color now represent species."}
#| code-fold: true
#| code-summary: "Show the code"
...
# I'm creating a scatter plot with mass and height on the x- and y-axes, and with color and point size representing species:
...
But – we still don't have an actual plot yet since we haven't told it what kind of plot to make.
...
▶️ Let's start with a simple bar plot using `geom_col()`:
...
This is the palette I used for the scatter plot earlier -- let's apply it to our bar graph too:
...
▶️ We can plot species on the x-axis but fill by name to get a stacked bar plot...
...
ggplot(
  ...
  aes(
    x = fct_reorder(species, desc(height)),
    y = height,
    fill= fct_reorder(name, desc(height)) # Sorting the fill factor by height also plots them in order by descending height within the grouping
  )
) +
  geom_bar(stat="identity") +
  scale_fill_viridis(option = "C", begin = 0.1, end = 0.9, discrete=TRUE) +
...
▶️ ...or use `position="dodge"` to plot the names side-by-side
...
That doesn't make much sense for height, but what if we wanted to show the sex distribution by species?
...
ggplot(
  aes(
    x=fct_reorder(name, height),
    y=height)) +
  geom_segment( aes(x=fct_reorder(name, height), xend=name, y=0, yend=height), color="deepskyblue") +
  geom_point(color="#2D708EFF", size=3.5, alpha=0.9) + 
  theme_classic() +
  labs(
    x = "Species",
    y = "Height (in cm)",
    title = "Star Wars Human and Droid Height"
  ) +
  coord_flip()
...
When you want to plot a variable's distribution, box plots are a good option.

However...

::: callout-warning
**Box plots can hide important trends in your data.**
:::

To illustrate this, I'm using a dummy dataset (from [this](https://www.data-to-viz.com/caveat/boxplot.html) excellent explanation of the dangers of box plots).

```{r}
#| echo: false
# create a dataset
# Set seed to avoid data being re-randomized every time
set.seed(100)
data <- data.frame(
...
There's a special composite plot called a Raincloud Plot that uses box plots, individual observations, *and* distribution plots to show all the information at once:
...
Histograms are like bar plots, but are for continuous instead of categorical variables.

Below, `height` is plotted on the x-axis, with the y-axis representing how many observations fall within each height range (or "bin"):
...
Changing the bin width can dramatically change how your data looks:
...
ggplot( 
  filtered_star_wars,
  aes(x=height)) +
  geom_histogram( binwidth=3, fill="#d2e8e3", color="#e9ecef", alpha=0.9) +
  theme_light() +
  theme(
    plot.title = element_text(size=15)
  ) +
    labs(
    x = "Star Wars Character Height",
    y = "Count",
    title = "Bin Width = 3"
  ) 
ggplot( 
  filtered_star_wars,
  aes(x=height)) +
  geom_histogram( binwidth=10, fill="#b4d9d0", color="#e9ecef", alpha=0.9) +
  theme_light() +
  theme(
    plot.title = element_text(size=15)
  ) +
...
ggplot( 
  filtered_star_wars,
  aes(x=height)) +
  geom_histogram( binwidth=50, fill="#548f81", color="#e9ecef", alpha=0.9) +
  theme_light() +
  theme(
    plot.title = element_text(size=15)
  ) +
...
ggplot( 
  filtered_star_wars,
  aes(x=height)) +
  geom_histogram( binwidth=100, fill="#345951", color="#e9ecef", alpha=0.9) +
  theme_light() +
  theme(
    plot.title = element_text(size=15)
  ) +
...
The bin width corresponds to the size of the grouping on the x-axis. For `binwidth=100` above, for example, each bin spans 100 cm.

▶️ Try it yourself using `mass` (weight) instead of `height` and try out different bin sizes.
...
These are like smoothed histograms (without the binning).

```{r fig.cap="`Density` is calculated using the *kernel density estimation*, and can be interpreted as the probability of an observation falling within a specific range of values on the x-axis. <br><br>The area under the density curve sums to 1 (a.k.a. 100% probability)."}
#| code-fold: true
#| code-summary: "Show the code"
ggplot( 
  filtered_star_wars,
  aes(x=height)) +
  geom_density(fill="#bbafda", color="#e9ecef", alpha=0.8) +
  theme_light() +
    labs(
    x = "Height (in cm)",
    y = "Density",
...
You can plot densities for several groups at once (this is possible with histograms too, but it's easier to interpret with density maps)

```{r}
#| code-fold: true
#| code-summary: "Show the code"
species_filt_starwars <- filtered_star_wars %>% 
  filter(species %in% c("Human","Droid", "Gungan"))

ggplot( 
  species_filt_starwars,
...
▶️ Make a density plot for `mass` -- how does it compare to your histogram?
...
Let's recreate a simple version of the one I showed you at the beginning of the lecture.

▶️ First plot `height` vs `mass`, mapping color onto `mass`
...
::: callout-note
When you map a variable to the size of the dot, a "scatter plot" technically becomes a "bubble plot."
:::

▶️ Let's map `species` to color and `birth year` to size to make a bubble plot:

```{r fig.cap= "This plot looks different because `birth year` is missing for several entries, so fewer points are plotted. <br><br>*That's a downside to plotting additional variables when there is missing data.*"}
#| code-fold: true
#| code-summary: "Show the code"
...
We'll close by making a heatmap.

This is one of the few instances where `ggplot2` isn't your best plotting option.

While you *can* use it to make heatmaps, the appropriately named `pheatmap` ("Pretty Heatmap") is best.

Here's a simple heatmap comparing `height` and `mass` in our dataset:

```{r}
#| echo: false
# Step 1: Prepare the data for heatmap
# Extracting the height and weight columns as matrix
heatmap_starwars <- species_filt_starwars %>% 
  filter(!is.na(mass))
...
# Step 2: Create the heatmap using pheatmap
pheatmap(
  heatmap_data,
  cluster_rows = FALSE,   # Cluster rows (optional)
  cluster_cols = FALSE,   # Do not cluster columns (optional)
  main = "Heatmap of Height and Weight"
)
```

This looks neat, but let's make some tweaks to make it more interpretable.

#### Scaling

First, `height` (cm) and `weight` (kg) are in different units, but the color scale treats them as being on the same continuous scale.

To address this, we can scale by column, centering and scaling each variable so that they have a mean of zero and standard deviation of one.

```{r fig.cap="Scaling is especially important for differential gene expression heatmaps because it allows you to better compare genes with very different expression levels."}
#| code-fold: true
#| code-summary: "Show the code"
pheatmap(
  heatmap_data,
...
The interpretation of the data doesn't *change* (the high values are still high, and the low values are still low), but now all the heights don't appear higher than all the weights.

::: {.callout-caution collapse="true"}
##### Caution when scaling

Be careful that you are scaling the appropriate values if you apply a scaling factor.

Look what happens if we scale by row instead of by column:
...
Because the mean of height is greater than the mean of weight (as they are on different scales), all the heights are greater than all the weights.
...
Let's also apply a clustering algorithm to determine which individuals are more similar, based on these two variables:
...
By looking at the dendogram on the left, we can get a greater appreciation for which individuals it groups by similarity.

For example, Padmé and Leia are grouped together, as are R2-D2 and R5-D4.

Hmm, that's interesting. Padmé and Leia are mother and daughter, and R2-D2 and R5-D4 are both the same kind of (astromech) droid.

So does that mean all the groupings are meaningful?

Well, we can also see that Darth Vader and IG-88 are grouped together. While Darth Vader is "more machine than man" according to Obi-Wan, he's still a human, but is being grouped with an assassin droid.

Plus, there's a whole mess of other groupings in the middle that are difficult to assess by eye.

#### Annotating

This is where annotations come in handy.

Let's create an annotation for species:
...
You can layer on multiple annotations -- can we glean more information by adding sex?

```{r}
#| code-fold: true
#| code-summary: "Show the code"
# Create an annotation dataframe for both `species` and `sex`
annotations <- data.frame(
  Species = heatmap_starwars$species,
  Sex = heatmap_starwars$sex
)
...
# Generate the heatmap with annotations for both `species` and `sex`
pheatmap(
  heatmap_data,
  annotation_row = annotations,  # Add row annotations for `species` and `sex`
  annotation_colors = annotation_colors,  # Add colors for the annotations
  cluster_rows = TRUE,  
  cluster_cols = FALSE,  
  scale="column"
...
#| code-fold: true
#| code-summary: "Show the code"
# Create an annotation dataframe for `species`, `sex`, and `homeworld`
heatmap_starwars$homeworld <- as.character(heatmap_starwars$homeworld)

heatmap_starwars <- heatmap_starwars %>%
  mutate(homeworld = replace_na(homeworld, "unknown"))

heatmap_starwars$homeworld <- as.factor(heatmap_starwars$homeworld)

annotations <- data.frame(
  Species = heatmap_starwars$species,
  Sex = heatmap_starwars$sex,
  Homeworld = heatmap_starwars$homeworld
)
rownames(annotations) <- heatmap_starwars$name # Match the rownames of the heatmap data
...
annotation_colors <- list(
  ...
  Homeworld = viridis(length(unique(heatmap_starwars$homeworld)), option = "C", begin = 1, end = 0) # Gradient for many levels
)
names(annotation_colors$Homeworld) <- unique(heatmap_starwars$homeworld) # Assign colors to each homeworld

# Generate the heatmap with annotations for `species`, `sex`, and `homeworld`
pheatmap(
  heatmap_data,
...
▶️ Recreate the first annotated heatmap yourself, but annotate with a different variable.

## Saving your Plots

You've now created some beautiful images -- but right now they're only being displayed in your Plots viewer in RStudio.

To keep them, we need to save them!

When creating a plot for lab meeting, you could save it as a `.png`, but for publication-quality plots, you really want to save them as a `.pdf`.

::: callout-tip
### Why save plots as a `.pdf`?

**Scalability:** PDFs are vector-based, meaning that you can resize plots without losing clarity.

**Quality Preservation:** PDFs maintain sharpness, avoiding the pixelation common in raster formats like PNG or JPG, making the the preferred option for presentations and detailed analysis.

*Taken from [this nice tutorial](https://tilburgsciencehub.com/topics/visualization/data-visualization/graphs-charts/saving-plots-r/).*
:::

To save a plot, you first have to create a variable that contains the plot information.

-   I haven't shown you how to do that so far because plot objects don't automatically render in the RStudio Plots viewer.
-   I typically wait to create a plot object until I'm finished tweaking a figure (resizing axes, changing colors, adjusting font size, etc.), just so I can quickly see how the changes are rendered.

**Pick your favorite plot from any point during the exercise.**

▶️ Copy the code, but now create a variable by creating a name and using the assignment operator `<-` before the `ggplot()` call:

```{r}
starwars_scatter <- ggplot(filtered_star_wars, 
                         aes(x = mass, 
                             y = height, 
                             color=mass)
                         ) +
...
# View the plot by calling the newly created plot object:
starwars_scatter
```

Our friend `ggplot2` has a handy function for saving plots easily, with lots of customization options.

One nice option is the `create.dir` argument, which allows you to specify the path to save the image, and create that directory if it doesn't already exist.

▶️ Let's save the plot as a `.png` and create a new folder in our project for figures:

```{r}
ggsave("starwars_scatterplot_weight_vs_age.png", 
       plot = starwars_scatter, 
       path = "output/figures", # Here's our new folder
       width = 10, 
       height = 6, 
       dpi = 300, 
       units = "in", 
...
It's also a good idea to time-stamp your figures so you know when they were created.

▶️ Let's automatically time-stamp the file title and save it as a `.pdf` instead:

```{r}
ggsave(filename = paste0("starwars_scatterplot_weight_vs_age_", format(Sys.Date(), "%Y-%m-%d"), ".pdf"),  
       plot = starwars_scatter, 
       path = "output/figures",
       width = 10, 
       height = 6, 
...
::: callout-note
You can leave the `create.dir` argument in your `ggsave()` call because it will only create the new directory if it doesn't already exist.

(In other words, it won't overwrite an existing directory, hooray!)
:::

Go ahead and look at your plots to see how they came out! 

One easy way to do this us using the Files navigator in RStudio:

![Navigate to the `output/figures` folder we just created within our R Project folder. Double click a file name to open it!](assets/Path_to_figures.png)

## Final Thoughts

By now, you should feel comfortable building a variety of plots in R.

Feel free to revisit this guide as you apply these skills to your own data!

::: callout-note
### Acknowledgements

I've indicated throughout this document where I've borrowed definitions, examples, etc. from other sources, but the below require special shout-outs:

-   [Yan Holtz](https://www.yan-holtz.com/)
    -   I've borrowed heavily from his excellent [R Graph Gallery](https://r-graph-gallery.com/). I intended to create my own cheat-sheet for generating common R plots, but his is so comprehensive that I will just point you to this resource.
    -   His [Productive R Workflow](https://www.productive-r-workflow.com/) course is the reason I was able to create this Quarto document and sync my local RStudio files with my GitHub. He offers discounts for groups, so contact me if you want to enroll; it's excellent.
-   [Matt Cannon](https://github.com/MVesuviusC) (MVesuviusC)
    -   His [R Workshop](https://github.com/MVesuviusC/R_workshop) GitHub page gave me ideas for how to structure the exercise.
    -   I used the variable and data type definitions from his [R Cheat Sheet](https://github.com/MVesuviusC/R_workshop/blob/main/cheatsheets/R_Cheat_Sheet.md), but his [Commonly Used R Functions Cheat Sheet](https://github.com/MVesuviusC/R_workshop/blob/main/cheatsheets/R_Functions_Cheat_Sheet.md) is a great resource too.
:::
