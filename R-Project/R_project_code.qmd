...
Use `??gt_theme` to explore available themes.
...
Since we already have a `none` category, and if we wanted to ensure our friend R2 was included in our hair color table, we could recode it so that an `NA` entry for hair becomes `none`.
:::
