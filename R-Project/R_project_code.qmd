...
It helps to think of the structure of a `ggplot` like adding layers to a "base" graph. 

All `ggplot`s start with, well, `ggplot()`!

▶️ Try just calling the `ggplot()` function on its own:
```{r fig.cap="This is like the canvas upon which your `ggplot` is built!"}
ggplot()
```

Next, you specify the data (either a Data Frame or Tibble) that you will be using for your plot. 

We'll stick with our `small_star_wars` Tibble for now to illustrate how to build the plot. 

If you just type `ggplot(small_star_wars)`, you'll get the same blank canvas. 

To specify *what* you want to plot, use the `aes()` argument. This is the *aesthetics* we are applying to our plot, and includes things like which variables to include and what colors should represent.
...
Ok, we have our canvas and we know what we want to plot. Now we have to decide how to plot it!

We need to layer on an argument to tell `ggplot()` what kind of plot we want. 

▶️ Let's make a simple bar plot:
```{r}
ggplot(
  small_star_wars,
  aes(
...
▶️ Let's color the bars by species using the `fill` argument:
```{r}
ggplot(
  small_star_wars,
  aes(
...
The default colors in `ggplot2` are not especailly colorblind-friendly.

:::

Using a tool called [Coblis](https://www.color-blindness.com/coblis-color-blindness-simulator/), you can simulate how different types of colorblindness affect how colors appear. 
...
To address this, I really like the `viridis` package, which has very nicely curated palettes that are all colorblind-friendly. They also read well in grayscale and B&W.

This is the palette I used for the scatterplot earlier:
...
▶️ Let's also get rid of `ggplot`'s default gray background, which adds unnecessary visual clutter. We can do this using a `theme()`:
...
You can see that R is doing its default behavior of ordering the variables on the x-axis alphabetically. 
...
Here's a simple code to automatically reorder a factor based on the value of another variable:

```{r}
# First we have to make `name` a factor:
small_star_wars$name <- as.factor(small_star_wars$name)

# Then we use `fct_reorder()` from the `forcats` package (which comes with the `tidyverse` package)
# We specify that we want them in descending order (ascending is the default)
ggplot(
  small_star_wars,
  aes(
...
▶️ Let's make ours clearer and more polished by setting them with `labs()`
```{r}
#| code-fold: true
#| code-summary: "Show the code"
ggplot(
