...
#### Lollipop Plots
You can make a type of modified bar plot called a lollipop plot:
```{r}
#| code-fold: true
#| code-summary: "Show the code"
ggplot(
...
```
:::{.callout-tip}
To improve readability for long categorical variable names, flip the x- and y-axes.

This also works if you have many individual samples. 

:::

▶️ Make a flipped-coordinate lollipop plot using `coord_flip()`
```{r}
#| code-fold: true
#| code-summary: "Show the code"
ggplot(
...
:::{.callout-warning}
Box plots are great for showing the distribution of a variable, **but you should very rarely (if ever) use a plain box plot.**
:::
This is because they can mask underlying trends in a distribution that are essential for proper data interpretation.
...
data %>%
  ggplot( aes(x=name, y=value, fill=name)) +
    ...
    xlab("")
```
Now let's plot each individual point too. 

To prevent the points from overlapping too much, we can use `geom_jitter()` to randomly disperse them a bit:
```{r}
#| code-fold: true
#| code-summary: "Show the code"
data %>% 
ggplot( aes(x=name, y=value, fill=name)) +
    ...
    xlab("")
```
Now, we can see that the sample size for group `C` is much smaller, and `B` seems to have a bimodal distribution, both of which are important to consider when interpreting the data.
...
If we want to really visualize the distribution of the data, we use a violin plot!
...
The plot above includes some code to add the sample size below each categorical value.

But there's a special plot called a Raincloud Plot that uses box plots, individual observations, *and* distribution plots to show all the information at once:
...
For your practice, let's use the full `starwars` dataset this time, to give us a larger sample size, and plot height by character sex, to give us more observations by group.
