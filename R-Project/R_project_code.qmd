...
You'll most often use **Data Frames** or **Tibbles** to create figures, but will use the variable types at one point or another.
...
But, oops, just kidding, I actually wanted `num` to be numeric. We can change it back:
...
It's because we entered the variable names in the logical order when we created the `rankings` variable. If we had created the variable with them out of order, `str()` would print them out of order, but R would still treat them alphabetically:
...
-   Single question mark: `?`
    ...
    -   Example: `?mean()` shows the documentation for Base-Rs `mean` function.
...
-   Double question mark: `??`
    ...
    -   This type of search is useful when you know *what* you want, but not the exact function name. It's also useful to see if there are multiple options with similar functions to choose from.
...
-   When loading a package, you don't need quotes around the name.
...
With `getwd()`, we're asking the question of what directory we're in, so we aren't giving it any arguments. 

But if we want to *change* the directory, we can use `setwd()`, giving it the path of the directory we want to move to:
...
Rather, they argue for making a folder for your specific project and then creating an R Project (or similar, but for our purposes we will discuss R Projects).

::: {.callout-tip collapse="true"}
##### What is an R Project?

-   A file with the `.Rproj` file extension that functions with RStudio and is associated with a directory
-   They “know” which files are relevant to a project: when you open the project, RStudio will load those files automatically
-   They automatically set your working directory to the project folder -- no need to use `setwd()` (and have our computers set on fire🔥)
-   This keeps files organized and reproducible, even if you move directories or switch computers.
...
However, RStudios defaults to *restoring your previous workspace*. Your previous workspace might include objects or data from a different project or version of your code, making it difficult to reproduce your results consistently. More on this [here](https://www.sesync.org/resources/tips-smooth-rstudio-workflow-and-reproducible-r-code).
...
I've put a ▶️ icon at each step where you will need to write in your script.

#### Install and Load Packages, Download Data

▶️ Please download and load the packages that we will use today (listed at the top of the script). 
...
▶️ Reading in the data should now look like:
```{r}
...
▶️ To get a general idea of what your Tibble looks like, you can use the `summary()` function:
```{r}
summary(small_star_wars)
```

There is a package called `gtExtras` that creates nice looking summary tables with very simple code:
```{r}
...
This code uses ` %>% `, which is called the `pipe operator`. 

[Pipes](https://www.geeksforgeeks.org/pipe-in-r/) are a bit more advanced than what we will be doing today, but their main benefit is to chain together multiple operations. 

Since we're only running one operation, you can just as easily generate the same table by using the `small_star_wars` variable as the argument passed to `gt()`:
```{r}
gt(small_star_wars)
```
...
:::{.callout-tip collapse=true}
##### A bit more on pipes
Here is a simple example of how pipes can be useful. 

If you want to apply a theme to make your `gtExtras` tables look like they do on the New York Times or ESPN, you could use either of these lines of code.
...
With `gtExtras`, you can do some cool things, like grouping specific variables together and plotting continuous variables right in your table (while applying some interesting visual styles): 
...
▶️ You don't have to get as fancy as that, but try making a table with a fun theme! 

To see the available themes, use the `??` keyword search: `??gt_theme`

#### `table()`
Remember when I used the `table()` function to show you that R was treating character variables alphabetically?

`table()` is actually a very useful function for creating frequency tables (showing you how many of one variable or a combination of variables are present).

Let's use this function to see how many of each species are in our dataset.

In R, when we want to select just one part of a Data Frame or Tibble, we use the `selection operator`, represented by `$`. 
...
What if we wanted to layer on some additional information? We can make a 2x2 table (or 3x3 in this case) by separating the two variables with a comma. 
...
:::{.callout-warning}
Note that R2-D2's hair (or lack thereof) is indicated in the dataset as `NA`:
...
When we create a table with `table()`, `NA`s are not included, **but there is no flag to alert us that an observation has been dropped due to `NA`s.**
...
Since we already have a `none` category, and if we really wanted to ensure our friend R2 was included in our hair color table, we could recode it so that an `NA` entry for hair becomes `none`.
:::
